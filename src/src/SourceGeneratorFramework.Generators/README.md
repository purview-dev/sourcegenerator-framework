...
`AttributeDataModelGenerator` generates `readonly record struct` parser models for attribute types. Instead of hand-writing repetitive `FromAttributeData` boilerplate, declare a partial model annotated with `[Generate]` and the generator fills in the rest.
...
- `[Generate(string targetAttribute)]` — same as above, but resolves the attribute by fully-qualified name. Use this when the attribute type is not available in the generator's compilation (e.g., `LengthAttribute` in .NET 8+ or a self-generated attribute).
