...
- **`SourceGeneratorTestRunner<TGenerator>`** — compiles a snippet of C# source, runs the generator,
...
- **`SourceGeneratorTestBase<TGenerator>`** — abstract base class that accepts an `ITestOutput`
...
Or derive from `SourceGeneratorTestBase<TGenerator>` and plug in your own `ITestOutput`
implementation.
...
`Get<T>`/`Has<T>`, syntax-tree lookup, and type-aware matching against `TypeReference`. Every `Get`
returns a `CodeQueryResult<T>` — the matched node (`Node`) plus a query scoped to it (`Query`) — with
